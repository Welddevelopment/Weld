...
export default function ComparisonSection() {
  ...
  // bodyRef is the SVG coordinate origin — SVG is a direct child of this element
  ...
  const measure = useCallback(() => {
    ...
    // SVG is position:absolute inside bodyRef, so bodyRef top-left = SVG (0,0)
    const origin = bodyRef.current.getBoundingClientRect()
  ...
  function buildPath(anchor: typeof LINE_ANCHORS[number]): string | null {
    ...
    // For left threads: connect from thread right edge → region left edge
    // For right threads: connect from thread left edge → region right edge
  ...
  return (
    <section className="cs-section" id="compare">
      ...
      {/* Body wrap: SVG coordinate origin — SVG + grid share this reference */}
      <div className="cs-body-wrap" ref={bodyRef}>

        {/* SVG lines — position:absolute inside body-wrap so (0,0) = body-wrap top-left */}
        <svg className="cs-lines" aria-hidden="true">
          {LINE_ANCHORS.map((anchor) => {
            const d = buildPath(anchor)
            if (!d) return null
            const active = activeThread !== null && activeThread === anchor.threadKey
            return (
