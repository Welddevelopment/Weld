...
# Define colors
DARK_BLUE = RGBColor(31, 71, 136)  # 1F4788
MEDIUM_BLUE = RGBColor(46, 92, 138)  # 2E5C8A
LIGHT_BLUE = RGBColor(62, 124, 184)  # 3E7CB8
DARK_GRAY = RGBColor(102, 102, 102)  # 666666
...
p = doc.add_paragraph("Stage 4 is the most substantial visual redesign in the Weld landing page overhaul series. Where Stages 1-3 changed system-level aesthetics (colors, typography, and motion), Stage 4 performs a deep visual redesign of each individual section using a specific Roblox-native UI metaphor.")

p = doc.add_paragraph("Every section of the landing page is mapped to a recognisable element of the Roblox ecosystem: Studio, Discord, the Explorer, the Properties panel. This design approach makes the page feel like it was built from inside the tools that Roblox developers already live in daily, creating immediate familiarity and trust.")

p = doc.add_paragraph("Each section redesign is a contained component change that can be shipped independently. The recommended implementation order (bottom of this document) prioritizes sections with the highest visual impact and developer resonance.")

# METAPHOR MAP
heading = doc.add_heading("The Metaphor Map", level=2)
heading.runs[0].font.color.rgb = MEDIUM_BLUE

p = doc.add_paragraph("Each landing page section is mapped to a Roblox UI element. The table below shows the current look, new metaphor, and inspiration source for every component redesign.")
...
p = doc.add_paragraph("The following sections detail the visual redesign for each component on the landing page. Each redesign provides specific implementation guidance including styling directives, element structure, and visual references.")
...
p = doc.add_paragraph("Each section redesign is a contained component change that can be shipped independently. The following order prioritizes sections with the highest visual impact and developer resonance.")
